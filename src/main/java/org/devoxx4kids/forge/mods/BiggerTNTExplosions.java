...
public class BiggerTNTExplosions 
{
	...
	{
		if(event.getEntityItem().getItem().getItem() != Item.getItemFromBlock(Blocks.TNT))
			return;
		
		EntityItem explosion = event.getEntityItem();
		event.getEntityItem().world.createExplosion(explosion, explosion.posX, explosion.posY, 
				explosion.posZ, power,  true);
	}
}
